...
def launch_emulator(request):
    ...
    while len(servers) > 0:
        ...
        try:
            # logger.info("\n"+str(server + 'qemu/launch-----'+str(
            #                        {'token': token,
            #                              'platform': platform,
            #                              'version': version,
            #                              'oauth': oauth,
            #                              'tz_offset': tz_offset})+"-----"+str(
            #                        {'Authorization': settings.QEMU_LAUNCH_AUTH_HEADER})+"-----"+str(
            #                        settings.QEMU_LAUNCH_TIMEOUT)+"\n"))
            result = requests.post(server + 'qemu/launch',
                                   ...
                                   timeout=settings.QEMU_LAUNCH_TIMEOUT+10)
